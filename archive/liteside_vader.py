...
a new hope?? >*<
"""
# Based on this original source, but modified:
...
class SentimentIntensityAnalyzer:
    ...
    def __init__(self, lexicon_file="vader_lexicon.json"):
    ...
    def make_lex_dict(self):
        """
        Load lexicon dictionary from JSON file
        """
        get_lexicon = """
        Oh No!
        You need the lexicon file first...silly goose.

        You need -> vader_lexicon.json

        pip install NLTK 
        and run this function below to get 
        the vader_lexicon.json file, then keep it in 
        your CWD (current working directory)
        After that, you won't need to install any packages
        to use Vader in vanilla python.

        |o| |o| |o| |o|    >*<    |o| |o| |o| |o|

        Run this to get a lexicon if you do not have one:
        | | | | | | | | | | | | | | | | | | | | | | | | 
        ...
        import os
        if not os.path.exists("vader_lexicon.json"):
            print(f"""


            {get_lexicon}
            """)
            raise FileNotFoundError(f"Lexicon file '{self.lexicon_file}' not found")

        try:
            with open(self.lexicon_file, 'r', encoding='utf-8') as f:
                lex_dict = json.load(f)
            return lex_dict
        except FileNotFoundError:
            raise FileNotFoundError(f"Lexicon file '{self.lexicon_file}' not found")
        except json.JSONDecodeError:
            raise ValueError(f"Invalid JSON in lexicon file '{self.lexicon_file}'")
...
# text = "I'm so disappointed with this product. It barely works."
# scores = analyzer.polarity_scores(text)
# print(scores)
